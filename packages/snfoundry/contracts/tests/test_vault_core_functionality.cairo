...
fn test_withdrawal_function_basic_operation() {
    ...
    assert!(user_collateral_after_deposit == WBTC_AMOUNT_LARGE, "User collateral should be recorded");
    ...
    assert!(final_user_vusd == user_vusd_after_deposit - burn_amount, "User vUSD should be reduced by burn amount");
    ...
    assert!(final_vault_vusd == user_vusd_after_deposit - burn_amount, "Vault issued vUSD should be reduced");
    assert!(final_user_collateral < user_collateral_after_deposit, "User collateral should be reduced");
}
...
fn test_withdrawal_function_full_withdrawal() {
    ...
    assert!(final_vault_wbtc <= WBTC_AMOUNT_SMALL / 1000, "Vault should have minimal WBTC locked after full withdrawal");
    assert!(final_vault_vusd == 0, "Vault should have no vUSD issued after full withdrawal");
    assert!(final_user_collateral <= WBTC_AMOUNT_SMALL / 1000, "User should have minimal collateral left after full withdrawal");
}
...
fn test_withdrawal_function_multiple_withdrawals() {
    ...
    assert!(vusd_after_first == initial_vusd - first_burn, "vUSD should be reduced after first withdrawal");
    ...
    // Verify cumulative effects
    assert!(final_vusd == initial_vusd - first_burn - second_burn, "Total vUSD burned should be sum of withdrawals");
...
fn test_withdrawal_function_multiple_users() {
    ...
    assert!(vault_wbtc_after_user1 < total_vault_wbtc_after_deposits, "Vault WBTC should be reduced");
    assert!(vault_vusd_after_user1 < total_vault_vusd_after_deposits, "Vault vUSD should be reduced");
    assert!(vault_vusd_after_user1 == user1_vusd_after + user2_vusd_after, "Vault vUSD should equal sum of user balances");
}
...
fn test_withdrawal_function_reverse_calculation_accuracy() {
    ...
    assert!(minted_vusd == calculated_vusd_from_wbtc, "Minted vUSD should match forward calculation");
