...
fn deploy_integration_test_system() -> (ContractAddress, ContractAddress, ContractAddress, ContractAddress) {
...
fn setup_user_wbtc(wbtc_address: ContractAddress, user_address: ContractAddress, vault_address: ContractAddress, amount: u256) {
...
fn test_complete_deposit_mint_workflow() {
    let (vault_address, vusd_address, wbtc_address, _oracle_address) = deploy_integration_test_system();
...
fn test_multiple_user_interactions() {
    ...
    assert!(total_vusd_issued == user1_vusd + user2_vusd, "Total vUSD should match individual amounts");
...
fn test_collateral_ratio_enforcement() {
    ...
    let actual_ratio = (collateral_value * 10000) / vusd_value; // Multiply by 10000 to get basis points
