...
import { getNetworkConfig, validateNetworkConfig, DEPLOYMENT_CONFIG } from "../config/networks";
...
export class VoltaDeployer {
  ...
  private async deployOracle(): Promise<void> {
    ...
    if (networkConfig.pragmaOracle) {
      ...
      console.log(green(`✓ Using existing Pragma Oracle: ${networkConfig.pragmaOracle}`));
  ...
  private async deployVault(): Promise<void> {
    ...
    if (!this.deployment.contracts.MockOracle?.address && !this.deployment.contracts.MockOracle?.address) {
      throw new Error("Oracle must be deployed before VoltaVault");
    }
    if (!this.deployment.contracts.MockWBTC?.address && !this.deployment.contracts.MockWBTC?.address) {
    ...
    this.deployment.contracts.VoltaVault = {
      address: result.address!,
      txHash: "deployed",
      constructorArgs: [deployer.address, vusdAddress, wbtcAddress, oracleAddress],
      timestamp: Date.now()
    };
  ...
  private async configureSystem(): Promise<void> {
    ...
    console.log(`   1. Set VoltaVault (${this.deployment.contracts.VoltaVault?.address}) as minter for vUSD`);
  ...
  private printDeploymentSummary(): void {
    ...
    console.log(`Timestamp: ${new Date(this.deployment.timestamp).toISOString()}`);
    console.log(``);
    ...
    console.log(`Max Price Deviation: ${this.deployment.config.maxPriceDeviation / 100}%`);
    console.log(`Min Collateral: ${this.deployment.config.minCollateralAmount / 100000000} WBTC`);
    console.log(blue(`━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━`));
  }
}