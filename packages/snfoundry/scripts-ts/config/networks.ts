...
export const networks: Record<string, NetworkConfig> = {
  sepolia: {
    ...
    rpcUrl: process.env.RPC_URL_SEPOLIA || "https://starknet-sepolia.public.blastapi.io/rpc/v0_9",
    explorerUrl: "https://sepolia.starkscan.co",
    
    // Pragma Oracle on Sepolia Testnet
    // Note: These are placeholder addresses - need to research actual addresses
    pragmaOracle: "0x036031daa264c24520b11d93af622c848b2499b66b41d611bac95e13cfca131a", // Placeholder - needs research
...
export function validateNetworkConfig(config: NetworkConfig): void {
  if (!config.accountAddress) {
    throw new Error(`Missing account address for network ${config.chainName}. Please set ACCOUNT_ADDRESS_${config.chainName.toUpperCase()} in .env`);
  }
  
  if (!config.privateKey) {
    throw new Error(`Missing private key for network ${config.chainName}. Please set PRIVATE_KEY_${config.chainName.toUpperCase()} in .env`);
  }
}
