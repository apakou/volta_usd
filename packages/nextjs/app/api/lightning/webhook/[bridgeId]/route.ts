...
import { chipiPayService, lightningOrchestrator } from '../../../../../services/lightning';
...
export async function POST(
  ...
  try {
    ...
    const isValidSignature = chipiPayService.verifyWebhookSignature(payload, signature);
    if (!isValidSignature) {
 ...
 */
async function handleInvoicePaid(event: ChipiPayWebhookEvent, bridgeId: string) {
  try {
    ...
    console.log(`Payment processed successfully for invoice: ${event.invoice_id}`);
 ...
 */
async function handleInvoiceExpired(event: ChipiPayWebhookEvent, bridgeId: string) {
  try {
 ...
 */
async function handleInvoiceCancelled(event: ChipiPayWebhookEvent, bridgeId: string) {
  try {
    console.log(`Invoice cancelled: ${event.invoice_id} for bridge: ${bridgeId}`);
    
    // Handle cancellation (similar to timeout)
    await lightningOrchestrator.handlePaymentTimeout(event.invoice_id);
    
    console.log(`Payment cancellation handled for invoice: ${event.invoice_id}`);
