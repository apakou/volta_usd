...
export class AtomiqService {
  ...
  async generateLightningInvoice(
    ...
    try {
      ...
      const invoice: LightningInvoice = {
        ...
        vusdAmount: params.vusdAmount,
        description: LightningUtils.generateInvoiceDescription(
          params.vusdAmount,
        ),
        status: "pending",
        ...
        createdAt: new Date(),
        expiresAt: new Date(
          Date.now() + LIGHTNING_CONFIG.lightning.defaultExpirySeconds * 1000,
        ),
        qrCode: await LightningUtils.generateQRCodeDataUri(
          response.bolt11 || "",
        ),
        deepLink: LightningUtils.generateLightningDeepLink(
          response.bolt11 || "",
        ),
      };
  ...
  async executeBridge(bridgeRequestId: string): Promise<AtomiqBridgeStatus> {
    try {
      const response = await this.apiCall(
        "POST",
        `/bridge/${bridgeRequestId}/execute`,
      );
   ...
   */
  private mapAtomiqStatus(
    atomiqStatus: string,
  ): "pending" | "lightning_paid" | "bridging" | "completed" | "failed" {
