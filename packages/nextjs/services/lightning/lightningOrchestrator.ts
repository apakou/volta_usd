...
export class LightningOrchestrator {
  ...
  async createPaymentFlow(params: {
    ...
    try {
      ...
      const invoice = await chipiPayService.createInvoice({
        vusdAmount: params.vusdAmount,
        btcPriceUsd: params.btcPriceUsd,
        description: params.description || `VOLTA USD Purchase - ${params.vusdAmount} VUSD`,
  ...
  async processPaymentCompletion(invoiceId: string): Promise<void> {
    try {
      ...
      const bridgeRequestId = await this.getBridgeRequestIdFromInvoice(invoiceId);
      
      // Step 3: Trigger Atomiq bridge execution
      await atomiqService.executeBridge(bridgeRequestId);
      
      console.log(`Lightning payment processed successfully for invoice: ${invoiceId}`);
    } catch (error) {
   ...
   */
  private async getBridgeRequestIdFromInvoice(invoiceId: string): Promise<string> {
  ...
  validatePaymentRequirements(params: {
    ...
    } else if (!LightningUtils.isValidStarknetAddress(params.userStarknetAddress)) {
