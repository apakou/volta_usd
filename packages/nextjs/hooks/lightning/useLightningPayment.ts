...
export const useLightningPayment = (
  options: UseLightningPaymentOptions = {},
): UseLightningPaymentReturn => {
  const {
    onPaymentComplete,
    onPaymentError,
    onStatusChange,
  ...
  // State
  const [paymentFlow, setPaymentFlow] = useState<LightningPaymentFlow | null>(
    null,
  );
  ...
  const formatAmount = useCallback(
    (amount: number, unit: "BTC" | "SATS" | "USD"): string => {
      switch (unit) {
        case "BTC":
