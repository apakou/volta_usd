...
export const useAtomiq = (options: UseAtomiqOptions = {}): UseAtomiqReturn => {
  ...
  const [bridgeRequest, setBridgeRequest] =
    useState<AtomiqBridgeRequest | null>(null);
  const [bridgeStatus, setBridgeStatus] = useState<AtomiqBridgeStatus | null>(
    null,
  );
  ...
  const getBridgeStatus = useCallback(
    async (bridgeId: string): Promise<AtomiqBridgeStatus> => {
      try {
        setIsLoading(true);
        setError(null);
      ...
      try {
        ...
        const atomiqError =
          err instanceof AtomiqError
            ? err
            : new AtomiqError(
                "Bridge cancellation failed",
                "BRIDGE_CANCELLATION_FAILED",
              );
  ...
  const getBridgeHistory = useCallback(
    async (starknetAddress: string): Promise<AtomiqBridgeRequest[]> => {
      try {
        setIsLoading(true);
        setError(null);
        ...
        const atomiqError =
          err instanceof AtomiqError
            ? err
            : new AtomiqError(
                "Failed to get bridge history",
                "BRIDGE_HISTORY_ERROR",
              );
