...
export const useTransactionWaiter = () => {
  ...
  const waitForTransaction = async (txHash: string, maxAttempts: number = 30, intervalMs: number = 2000): Promise<boolean> => {
    if (!provider || !txHash) {
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
      try {
        console.log(`Transaction status check attempt ${attempt}/${maxAttempts}`);
        ...
        console.log(`Transaction not yet confirmed (attempt ${attempt}/${maxAttempts}):`, error);
        
        // If it's the last attempt, return false
        if (attempt === maxAttempts) {
          console.error("Transaction confirmation timeout after maximum attempts");
          return false;
        }
