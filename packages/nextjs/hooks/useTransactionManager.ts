...
export const useTransactionManager = () => {
  ...
  const trackTransaction = useCallback(
    async (txHash: string) => {
      ...
      setActiveTransactions((prev) =>
        prev.map((tx) =>
          tx.txHash === txHash ? { ...tx, status: "confirming" as const } : tx,
        ),
      );
      ...
      try {
        ...
        if (confirmed) {
          ...
          setActiveTransactions((prev) =>
            prev.map((tx) =>
              tx.txHash === txHash ? { ...tx, status: "failed" as const } : tx,
            ),
          );
          ...
          setTimeout(async () => {
            console.log(
              "Refreshing balances after failed transaction (fallback)...",
            );
            await refetchBalances();
          }, 3000);
        }
        ...
        setTimeout(async () => {
          console.log(
            "Refreshing balances after transaction error (fallback)...",
          );
          await refetchBalances();
        }, 5000);
      }
  ...
  const hasActiveMinting = activeTransactions.some(
